            ...
            {filteredProducts.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 auto-rows-fr pb-8">
                    {filteredProducts.map(product => (
                        <div key={product.id} className="group flex flex-row md:flex-col bg-white dark:bg-[#1a1a1a] border-4 border-black dark:border-white hover:border-primary dark:hover:border-primary transition-all duration-300 hover:translate-x-1 md:hover:translate-x-0 md:hover:-translate-y-1 hover:shadow-[4px_4px_0px_0px_#0000FF] md:hover:shadow-[8px_8px_0px_0px_#0000FF] relative overflow-hidden h-auto md:h-full items-stretch">

                            {/* Image Area */}
                            <div className="relative w-32 md:w-full shrink-0 md:aspect-[4/3] border-r-4 md:border-r-0 md:border-b-4 border-black dark:border-white bg-gray-200 dark:bg-black overflow-hidden flex flex-col">
                                <img
                                    src={product.image}
                                    alt={product.name}
                                ...
                                />
                                {/* Stock Overlay */}
                                <div className="absolute bottom-0 left-0 right-0 bg-black/60 md:bg-gradient-to-t md:from-black/80 md:to-transparent p-1 md:p-4 pt-1 md:pt-10 flex justify-center md:justify-between items-end">
                                    <div className={`px-1.5 py-0.5 md:px-2 border md:border-2 ${product.stock <= (product.minStock || 5) ? 'bg-red-600 border-red-600 text-white animate-pulse' : 'bg-black/50 border-white text-white'}`}>
                                        <span className="text-[8px] md:text-[10px] font-black uppercase text-center block">
                            ...
                            {/* Content */}
                            <div className="p-3 md:p-4 flex flex-col flex-1 gap-2 md:gap-3 justify-between">
                                <div className="flex flex-col gap-1">
                                    <h2 className="text-black dark:text-white text-sm md:text-xl font-black uppercase leading-tight line-clamp-3 md:line-clamp-2" title={product.name}>
                                        {product.name}
                                    </h2>
                                    <div className="w-8 md:w-12 h-1 bg-primary mt-1 hidden md:block"></div>
                                </div>

                                {/* Materials Mini-BOM - Hidden on Mobile */}
