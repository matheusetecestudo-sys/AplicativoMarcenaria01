    ...
    return (
        <div className="relative h-full flex flex-col pb-8">
            ...
            {filteredProducts.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 md:gap-6 auto-rows-fr pb-8">
                    {filteredProducts.map(product => (
                        <div key={product.id} className="group flex flex-row md:flex-col bg-white dark:bg-[#1a1a1a] border-4 border-black dark:border-white hover:border-primary dark:hover:border-primary transition-all duration-300 relative overflow-hidden h-auto md:h-full shadow-sm hover:shadow-[8px_8px_0px_0px_rgba(0,0,255,0.2)]">
                            ...
                            <div className="relative w-32 md:w-full shrink-0 md:aspect-[4/3] border-r-4 md:border-r-0 md:border-b-4 border-black dark:border-white bg-gray-100 dark:bg-black overflow-hidden flex flex-col justify-center">
                                ...
                                />
                                {/* Stock Badge Overlay */}
                                <div className="absolute top-2 left-2 md:top-auto md:bottom-3 md:left-3">
                                    <div className={`px-2 py-0.5 border-2 shadow-sm ${product.stock <= (product.minStock || 5) ? 'bg-red-600 border-red-600 text-white animate-pulse' : 'bg-white border-black text-black'}`}>
                                        <span className="text-[9px] md:text-[10px] font-black uppercase">
                                            {product.stock <= (product.minStock || 5) ? 'Baixo' : 'Ok'}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* CONTENT SECTION */}
                            <div className="p-3 md:p-4 flex flex-col flex-1 gap-2 justify-between">

                                {/* HEADER INFO */}
                                <div className="flex flex-col gap-1">
                                    <h2 className="text-black dark:text-white text-sm md:text-xl font-black uppercase leading-none md:leading-tight line-clamp-2">
                                        {product.name}
                                    </h2>
                                    <p className="text-xs md:text-base font-medium text-gray-500 dark:text-gray-400">
                                        <span className="font-mono font-bold text-black dark:text-white">R$ {product.cost.toFixed(2)}</span> /un
                                    </p>
                                </div>

                                {/* DESKTOP: MATERIALS */}
                                <div className="hidden md:block flex-1 mt-2">
                                    <p className="text-[10px] font-bold uppercase text-gray-400 mb-1">Materiais</p>
                                    <div className="bg-gray-50 dark:bg-black/50 border-l-2 border-gray-300 dark:border-gray-700 p-2 text-xs font-mono h-[60px] overflow-y-auto custom-scrollbar">
                                        {product.materials.length > 0 ? product.materials.map(m => <div key={m} className="truncate">{m}</div>) : <span className="text-gray-400 italic">--</span>}
                                    </div>
                                </div>

                                {/* STOCK CONTROLS & ACTIONS (Unified) */}
                                <div className="flex items-center justify-between mt-auto pt-2 md:pt-4 border-t-0 md:border-t-2 border-dashed border-gray-300 dark:border-gray-800">

                                    {/* Stock Counter */}
                                    <div className="flex items-center gap-1 bg-gray-100 dark:bg-black/50 p-1 rounded-sm border border-gray-200 dark:border-gray-700">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); updateProductStock(product.id, -1) }}
                                            className="size-6 flex items-center justify-center bg-white dark:bg-[#222] hover:bg-red-500 hover:text-white transition-colors border border-gray-300 dark:border-gray-600 rounded-sm"
                                        >-</button>
                                        <span className="w-8 text-center font-mono font-bold text-sm text-black dark:text-white">{product.stock}</span>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); updateProductStock(product.id, 1) }}
                                            className="size-6 flex items-center justify-center bg-white dark:bg-[#222] hover:bg-primary hover:text-white transition-colors border border-gray-300 dark:border-gray-600 rounded-sm"
                                        >+</button>
                                    </div>
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="size-8 flex items-center justify-center text-gray-400 hover:text-black dark:hover:text-white transition-colors"
                                        >
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-fade-in-up" onClick={closeModal}>
                        <div className="bg-white dark:bg-[#0A0A0A] w-full max-w-4xl border-4 border-primary shadow-[12px_12px_0px_0px_rgba(0,0,255,0.5)] flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                            ...
                            `}
                                ...
                                <div className="p-4 md:p-8 flex flex-col bg-gray-50 dark:bg-[#000] min-h-[500px] md:min-h-0 h-full overflow-hidden relative">
