...
export const Orders: React.FC = () => {
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 lg:h-full h-auto pb-8 items-start">
            ...
            {/* LEFT COLUMN: ORDER BUILDER (PDV Style) - REMOVED STICKY TO PREVENT CLIPPING */}
            <div className={`xl:col-span-6 flex flex-col transition-all relative bg-white dark:bg-[#1A1A1A] border-4 border-black dark:border-white shadow-[12px_12px_0px_0px_#0000FF] ${isBuilderExpanded ? 'mb-8' : 'mb-0'}`}>
                ...
                <div className="p-2 border-b-4 border-black dark:border-white bg-white dark:bg-[#1A1A1A] shrink-0">
                    <div className="flex justify-between items-center mb-1">
                        ...
                        <button
                            ...
                            className="lg:hidden p-1 border-2 border-black dark:border-white rounded"
                        >
                    ...
                    <div className={`${isBuilderExpanded ? 'grid' : 'hidden'} lg:grid grid-cols-1 md:grid-cols-2 gap-2`}>
                        <div className="md:col-span-2 relative">
                            <input
                                className="w-full bg-gray-50 dark:bg-black text-black dark:text-white px-3 py-1.5 text-xs font-black border-2 border-black dark:border-white focus:border-primary focus:outline-none uppercase"
                        ...
                        <div className="flex items-center bg-gray-100 dark:bg-black border-2 border-black dark:border-white h-8 p-0.5 relative">
                            <div
                                className={`absolute top-0.5 bottom-0.5 w-[calc(50%-2px)] transition-all duration-300 border border-black
                            ${origin === 'ONLINE' ? 'left-0.5 bg-[#00FFFF]' : 'left-[calc(50%+0.5px)] bg-[#FFA500]'}`}
                            ></div>
                            <button onClick={() => setOrigin('ONLINE')} className="flex-1 z-10 text-[8px] font-black uppercase text-center">Online</button>
                            <button onClick={() => setOrigin('FISICO')} className="flex-1 z-10 text-[8px] font-black uppercase text-center">Loja</button>
                        </div>
                    </div>
                </div>

                {/* BODY WRAPPER */}
                <div className={`${isBuilderExpanded ? 'flex' : 'hidden'} lg:flex flex-col flex-1 min-h-0`}>
                    {/* 2. ITEM ENTRY TOOLBAR */}
                    <div className="bg-gray-50 dark:bg-black p-2 flex flex-col gap-1 border-b-4 border-black dark:border-white shrink-0">
                        <div className="grid grid-cols-12 gap-1 px-1">
                            <div className="col-span-9 md:col-span-10">
                                <select
                                    className="w-full h-8 bg-white dark:bg-[#111] text-black dark:text-white px-2 font-black border-2 border-black dark:border-white text-[10px] uppercase"
                            ...
                            <div className="col-span-3 md:col-span-2">
                                <input
                                    type="number"
                                    min="1"
                                    className="w-full h-8 bg-white dark:bg-[#111] text-black dark:text-white text-center font-black border-2 border-black dark:border-white text-xs"
                                    value={quantity}
                        ...
                        <div className="flex gap-1 px-1">
                            ...
                            <button
                                ...
                                className="w-10 h-8 bg-primary text-white flex items-center justify-center hover:brightness-110 active:scale-95 border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_#000]"
                            >
                    ...
                    {/* 3. CART LIST - Standard height on desktop, flexible on mobile */}
                    <div className="bg-white dark:bg-[#0A0A0A] p-0 relative min-h-[150px] max-h-[400px] overflow-y-auto custom-scrollbar">
                        {currentItems.length === 0 ? (
                            <div className="flex flex-col items-center justify-center text-gray-300 h-32 p-4">
                                <span className="material-symbols-outlined text-3xl opacity-5">shopping_cart</span>
                                <p className="font-black uppercase text-[7px] tracking-widest opacity-10 text-center">SEM PRODUTOS</p>
                            </div>
                        ) : (
                                ...
                                <tbody className="text-[11px]">
                                    {currentItems.map((item, idx) => (
                                        <tr key={idx} className="border-b border-gray-100 dark:border-gray-900">
                                            <td className="py-2 px-3 text-black dark:text-white font-black uppercase leading-tight">
                                                {item.productName}
                                                <div className="text-[8px] text-gray-400 font-bold">{item.quantity} UN @ R$ {item.unitPrice.toFixed(2)}</div>
                                            </td>
                                            <td className="py-2 px-3 text-right font-black">R$ {item.total.toFixed(2)}</td>
                                            <td className="py-2 px-1 text-center">
                                                <button onClick={() => removeItemFromCart(idx)} className="text-red-500"><span className="material-symbols-outlined text-xs">close</span></button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                    ...
                    {/* 4. FOOTER */}
                    <div className="p-3 bg-white dark:bg-[#1A1A1A] border-t-8 border-black dark:border-white">
                        ...
                        <div className="flex gap-3 items-stretch h-14">
                            <div className="flex-[1.3] bg-black dark:bg-white text-white dark:text-black p-2 flex flex-col justify-center border-2 border-black shadow-[4px_4px_0px_0px_#0000FF]">
                                <span className="text-[9px] font-black uppercase opacity-50">Total Geral</span>
                                <span className="text-xl font-black">R$ {calculateCartTotal().toFixed(2)}</span>
                            </div>
                ...
                <div className="flex-1 overflow-auto custom-scrollbar">
                    <div className="flex flex-col gap-6">
                        {filteredOrders.length === 0 ? (
                            ...
                            const isLate = order.status !== 'CONCLUÍDO' && order.status !== 'CANCELADO' && deadlineStr < todayStr;
